...
# Please remeber to install youtube-dl and python-vlc
# C:/Python27/python.exe -m pip install -U <package>
import pafy
import vlc
...
class Context():
	def __init__(self, node):
		...
		self.CustomRequestHandlers				= {
            'play':                     self.PlayHandler,
            'stop':                     self.StopHandler,
            'pause':                    self.PauseHandler,
            'vol_up':                   self.VolumeUpHandler,
            'vol_down':                 self.VolumeDownHandler,
		}
		...
		self.PlayURL 					= None
		# VLC
		self.VLCInstance				= None # vlc.Instance('--novideo')
		self.Player 					= None
		self.Media 						= None
	...
	def PlayHandler(self, sock, packet):
		print ("PlayHandler")
    
	def StopHandler(self, sock, packet):
		print ("StopHandler")
    
	def PauseHandler(self, sock, packet):
		print ("PauseHandler")
    
	def VolumeUpHandler(self, sock, packet):
		print ("VolumeUpHandler")
    
	def VolumeDownHandler(self, sock, packet):
		print ("VolumeDownHandler")
