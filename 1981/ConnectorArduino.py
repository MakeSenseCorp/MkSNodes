...
class Connector (MkSAbstractConnector.AbstractConnector):
	...
	def SetSensorInfo(self, info):
		...
		txPacket = self.Protocol.SetArduinoNanoUSBSensorValueCommand(jsonInfo["id"], jsonInfo["value"])
		rxPacket = self.Adaptor.Send(txPacket)
	...
	def GetSensorInfo(self, info):
		jsonInfo = json.loads(info)
		txPacket = self.Protocol.GetArduinoNanoUSBSensorValueCommand(jsonInfo["Id"])
		rxPacket = self.Adaptor.Send(txPacket)
		if (len(rxPacket) > 6):
			MagicOne, MagicTwo, Opcode, Length, Id, Value = struct.unpack("BBHBBH", rxPacket[0:8])
			return json.loads("{\"status\":\"OK\",\"payload\":{\"id\":" + str(Id) + ",\"value\":" + str(Value) + "}}")
		else:
			return json.loads("{\"status\":\"FAILED\"}")

	def GetSensorListInfo(self):
		txPacket = self.Protocol.GetDeviceInfoSensorsCommand()
		rxPacket = self.Adaptor.Send(txPacket)

		if (len(rxPacket) > 7):
			MagicOne, MagicTwo, Opcode, Length, SensorsCount = struct.unpack("BBHBB", rxPacket[0:6])

			payload = "\"sensors\":["
			for i in range(0,SensorsCount):
				pin, id, value, group, direction = struct.unpack("BBBBB", rxPacket[6 + (5 * i):6 + (5 * (i + 1))])
				payload += "{\"id\":" + str(id) + ",\"value\":" + str(value) + ",\"group\":" + str(group) + ",\"direction\":" + str(direction) + "},"
			payload = payload[0:-1] + "]"
			ret = "{\"status\":\"OK\",\"payload\":{" + payload + "}}"
			return json.loads(ret)
